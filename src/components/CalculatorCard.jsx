...
export function CalculatorCard(props) {
    ...
    const fetchDados = async () => {
      try {
        const resposta = await fetch(
          "http://3.142.149.185:8080/api/medications"
        );
  ...
  function handleApresentacaoChange(e) {
    const apresentacao = apresentacoes.find(
      (a) => a.idPresentation === e.target.value
    );
  ...
  const buttonCalcularClass = `calcular${
    !isCalculateDisabled() ? " ativo" : ""
  }`;
      ...
      medications: [
        {
          ...
          dosage: textoFinal,
          schedule: values.intervalo
            ? `a cada ${values.intervalo} horas`
            : null,
        },
      ],
    };

    const calculosExistentes = JSON.parse(
      localStorage.getItem("calculos") || "[]"
    );
        ...
        <div className="content">
              ...
              <div className="select-apresentacao-fav">
                <select onChange={handleApresentacaoChange} defaultValue="">
                  ...
                  {apresentacoes.map((ap, index) => (
                    <option
                      key={ap.idPresentation || index}
                      value={ap.idPresentation}
                    >
          ...
          {calculo && (
            <div className="result_calculate">
              ...
              <button className="cancelar" type="button" onClick={calculate}>
                Calcular
              </button>
            </div>
          )}
          ...
          <div className="buttons">
            ...
            {calculo && (
              <button className="cancelar" type="button" onClick={addDuracao}>
                Adicionar à receita
              </button>
            )}
          </div>
